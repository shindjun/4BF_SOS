...
charge_rate = 60 / charging_time_per_charge  # 분당 Charge 수
...
nc_per_charge = st.sidebar.number_input("너트코크(N.C) 장입량 (kg/ch)", value=0.0)

# O/C 비율 입력
oc_ratio = st.sidebar.number_input("O/C (Ore/Coke 비율)", value=5.0)
...
# 비상체류시간 보정 (비상조업 적용 시만 반영)
if abnormal_active and adjusted_elapsed_minutes > abnormal_total_melting_delay:
    active_minutes = adjusted_elapsed_minutes - abnormal_total_melting_delay
...
st.sidebar.header("⑤ 실측출선 및 저선량 입력")
...
st.sidebar.header("⑥ 출선 전략 및 시간 예측")

# TAP당 평균 출선시간 입력
fixed_avg_tap_time = st.sidebar.number_input("TAP당 평균 출선시간 (분)", value=252.0)

# 선행 출선 종료 예상시각 계산
lead_target = fixed_avg_tap_output  # 목표 용선 출선량
...
# ========================== 7부: 예상 일일 생산량 + 용선온도 예측 ==========================

# 예상 일일 생산량 (송풍 기준 공식 적용)
...
# 환원효율 기반 보조 생산량 계산 (경과시간 고려)
if adjusted_elapsed_minutes > 0 and elapsed_charges > 0:
...
# 미분탄 ton/hr 환산 (용선온도 보정 참고용)
pci_ton_hr = pci_rate * daily_production_by_wind / 1000

# 용선온도 예측 모델 (예측이 필요 없는 경우 생략 가능)
Tf_predict = None  # 최종에서 예측 온도 제거된 상태라면 생략하거나 고정값
