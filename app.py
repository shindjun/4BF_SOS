...
# 페이지 설정
st.set_page_config(
    page_title="BlastTap 10.3 Pro — AI 고로조업 실시간 통합관리",
    layout="wide"
)
...
charge_rate = 60 / charging_time_per_charge  # charge/hr
...
nut_coke = st.sidebar.number_input("너트코크 (N.C) 장입량 (kg)", value=8160.0)

# O/C 비율 (ORE/COAL)
oc_ratio = st.sidebar.number_input("ORE / COKE 비율 (O/C)", value=4.95)
...
if abnormal_active:
    ...
    abnormal_wind_unit = st.sidebar.number_input("비상 송풍원단위 (Nm³/t)", value=1189.0)
...
if reduction_active:
    ...
    reduction_wind_unit = st.sidebar.number_input("감풍 송풍원단위 (Nm³/t)", value=1189.0)

# ========================== 4부: 시간분할 환원효율 및 생산량 계산 ==========================

# 기본 정상조업 환원효율 계산
size_effect = 1.0  # 입도 영향 생략
...
# 선행 출선 종료 후 차기 출선 간격
next_tap_gap = st.sidebar.number_input("선행 종료 후 차기 출선간격 (분)", value=15.0)
...
record = {
    ...
    "저선량": residual_molten,
    "저선율": residual_rate,
    "송풍생산량": daily_production_by_wind,
    "환원생산량": daily_production_est,
    "공취간격": gap_minutes,
    "수지차": residual_gap,
    "상태": status
}
