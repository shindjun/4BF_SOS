...
elapsed_minutes = max(min(elapsed_minutes, 1440), 60)  # 1일 최대 1440분 제한

st.sidebar.header("① 정상조업 입력")

# 장입시간 (1Charge당)
charging_time_per_charge = st.sidebar.number_input("1Charge 장입시간 (분)", value=11.0)
charge_rate = 60 / charging_time_per_charge  # 시간당 Charge 수
...
nut_coke = st.sidebar.number_input("너트코크 N.C 장입량 (kg)", value=800.0)
...
# 조업지수
reduction_efficiency = st.sidebar.number_input("기준 환원율", value=1.0)
...
# 송풍원단위 (생산량 보조식 기준)
wind_unit = st.sidebar.number_input("송풍원단위 (Nm³/ton)", value=1189.0)
...
if reduction_active:
    reduction_start_time = st.sidebar.time_input("감풍 시작 시각", value=datetime.time(15, 0))
    reduction_end_time = st.sidebar.time_input("감풍 종료 시각", value=datetime.time(18, 0))
    ...
    reduction_pci_rate = st.sidebar.number_input("감풍 미분탄 취입량 (kg/thm)", value=pci_rate)
    reduction_wind_unit = st.sidebar.number_input("감풍 송풍원단위 (Nm³/t)", value=wind_unit)

# ========== 4부: 환원효율 계산 및 시간가중 생산량 계산 ==========

# 각 인자 기반 정상조업 환원효율 계산
size_effect = (20 / 20 + 60 / 60) / 2  # 입도효과
...
lead_elapsed_time = st.sidebar.number_input("선행 출선소요시간 (분)", value=90.0)
follow_elapsed_time = st.sidebar.number_input("후행 출선소요시간 (분)", value=30.0)
...
# AI 계산 누적 생산량 (예상 일일생산량 * 경과시간 비율)
production_ton_ai = daily_production_by_wind * (elapsed_minutes / 1440)

# AI 기반 저선량 = 생산량 - 누적 출선량
...
# 현재시각 기준 예상 누적생산량 (송풍기준 보정)
expected_now_production = (elapsed_minutes / 1440) * daily_production_by_wind
...
st.write(f"예상 누적 생산량 (현재시각 기준): {expected_now_production:.1f} ton")

# 출선 및 저선량 요약
st.subheader("🛢️ 출선 현황 및 잔류물")
...
st.write(f"예상 현재 생산량: {total_production_ton:.1f} ton")
st.write(f"예상 현재 저선량: {residual_molten:.1f} ton ({residual_rate:.2f}%)")
st.write(f"실측 저선 입력값: {measured_residual_molten:.1f} ton")
st.write(f"AI-실측 저선 수지차: {residual_gap:.1f} ton")
...
record = {
    ...
    "예상누적생산량": total_production_ton,
    "종료Tap출선": tap_total_output,
    "선행출선": lead_output,
    "후행출선": follow_output,
    "누적출선": total_tapped_hot_metal,
    "예상저선량": residual_molten,
