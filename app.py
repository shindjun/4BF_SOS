...
nc_per_charge = st.sidebar.number_input("너트코크(N.C) 장입량 (kg/ch)", value=120.0)

# Ore/Coke 비율
ore_coke_ratio = st.sidebar.number_input("ORE/COKE 비율 (O/C)", value=5.0)
...
if abnormal_active:
    ...
    total_melting_delay = st.sidebar.number_input("비상 체류시간 보정 (분)", value=300)
...
if abnormal_active:
    ...
    abnormal_reduction_eff = (
        ...
        abnormal_temp_effect * abnormal_pci_effect * abnormal_measured_temp_effect * 0.9
    )
else:
...
# ========================== 5부: 실측 출선 + 저선량 + 슬래그 + 실측 저선량 입력 ==========================

st.sidebar.header("④ 실측출선 데이터 입력")

# TAP 평균 출선량 및 종료된 Tap 수 입력
...
# 차기 출선간격 수동입력 항목
manual_next_tap_gap = st.sidebar.number_input("선행 폐쇄 후 차기 출선간격 (분)", value=15)
...
# 용선온도 예측 (AI 예측 제거됨 – 단순화된 처리로 보류 가능)
Tf_predict = 0  # 사용하지 않으므로 초기화만 함
...
record = {
    ...
    "예상일일생산량_송풍기준": daily_production_by_wind,
    "예상일일생산량_환원효율": daily_production_est,
    "누적출선량": total_tapped_hot_metal,
    "AI저선량": residual_molten,
    "저선율": residual_rate,
    "공취예상시간": gap_minutes,
    "출선소요시간": expected_tap_time,
    "저선상태": status
}
