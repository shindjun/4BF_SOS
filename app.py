...
elapsed_minutes = max(min(elapsed_minutes, 1440), 60)  # 1시간 ~ 24시간 제한
...
charge_rate = 60 / charging_time_per_charge  # ch/h
...
nut_coke_kg = st.sidebar.number_input("너트코크 장입량 (kg/ch)", value=18.0)

# O/C 비율
ore_coke_ratio = st.sidebar.number_input("O/C 비율 (ORE/CO)", value=5.0)
...
production_ton_ai = total_production_ton
...
fixed_avg_tap_time = st.sidebar.number_input("TAP당 평균출선시간 (분)", value=252.0)
...
# 차기 출선간격 (참고입력)
st.sidebar.number_input("선행폐쇄 후 차기 출선간격 (분)", value=15)
...
# ========================== 7부: 송풍원단위 기반 예상 일일생산량 ==========================

# 예상 일일생산량 계산 (풍량×1440 + 산소×24/0.21) / 송풍원단위
...
# 예상 일일생산량 = 단일 기준 (통일)
daily_production_est = daily_production_by_wind
...
# 생산량 요약
st.write(f"예상 일일생산량 (송풍기준): {daily_production_est:.1f} ton/day")
...
# 평균 출선시간 입력이 있을 경우 추가 표시
st.write(f"TAP당 평균 출선시간 입력값: {tap_avg_duration:.1f} 분")
st.write(f"선행 폐쇄 후 차기 출선간격 추천: {next_tap_interval}")
