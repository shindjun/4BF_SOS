...
elapsed_minutes = max(min(elapsed_minutes, 1440), 0)  # 하루 1440분 제한
...
daily_expected_production = blast_volume * 60 * 24 / wind_unit  # 송풍기준 일일생산량
expected_till_now = daily_expected_production * elapsed_ratio  # 현재시각까지 누적 생산량

# 🔸 현재 시각 기준 저선량 = 예상 누적 생산량 - 누적 출선량
residual_molten = expected_till_now - total_tapped_hot_metal
residual_molten = max(residual_molten, 0)

# 🔸 슬래그 자동계산 (슬래그비율은 고정값 사용)
...
record = {
    ...
    "현재시각까지예상생산량(t)": expected_till_now,
...
st.markdown("- 주요기능: 🔹 일일생산량 예측 🔹 실시간 저선관리 🔹 출선전략 시각화 🔹 CSV 저장")

st.info("💡 모든 조업 정보는 매일 **07시 기준으로 리셋**되며,\n실시간 출선소요량 기반으로 누적 생산량이 자동 보정됩니다.")

st.success("📌 **BlastTap 10.3 Pro**는 현재 *베타 운영 중*이며, 조업 안정성과 자동화 연동 고도화를 목표로 지속 개선됩니다.")
