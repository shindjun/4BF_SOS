...
# 페이지 기본 설정
st.set_page_config(
    page_title="BlastTap 10.3 Pro — AI 실시간 조업 리포트",
    layout="wide"
)

# 타이틀
st.title("📊 BlastTap 10.3 Pro — AI 실시간 조업 리포트")
...
# 현재 장입 진행 수 (수동 입력)
current_charges = st.sidebar.number_input("현재 Charge 수 (수동입력)", value=65)
planned_charges = st.sidebar.number_input("일일 계획 Charge 수", value=124)
...
# 환원효율 기본요소 계산
size_effect = (20 / 20 + 60 / 60) / 2  # 고정 비율
...
tap_total_output = fixed_avg_tap_output * completed_taps  # 실측 TAP 총 출선량
...
st.write(f"일일예상생산량: {daily_production_by_wind:.1f} ton/day")
